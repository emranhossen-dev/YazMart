...
export default async function SellerCategoriesPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-zinc-950">Category Directory</h1>
        <p className="text-xs font-semibold text-zinc-400">Browse platform-wide categories to map your products catalog.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {categories.length === 0 ? (
          ...
          categories.map((category) => (
            <div key={category.id} className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm space-y-2 flex flex-col justify-between">
              <div className="flex items-center gap-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-100 text-zinc-600">
                  <FolderHeart className="h-4 w-4" />
                </div>
                <h4 className="font-extrabold text-sm text-zinc-900 truncate">{category.name}</h4>
              </div>
              <p className="text-[10px] text-zinc-400 font-mono select-all truncate">ID: {category.id}</p>
            </div>
          ))
        )}
      </div>
