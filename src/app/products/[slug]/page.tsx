...
import { ShoppingCart, Heart, Shield, Package, ArrowLeft, RefreshCw, ShoppingBag } from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  const { cart, wishlist, addToCart, toggleWishlist, updateQuantity } = useShopStore();
  ...
  const loadDetails = async () => {
    ...
    if (res.product) {
      ...
      setActiveImage(res.product.featured_image || "");
    }
    setLoading(false);
  };
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[var(--background)] gap-3">
        ...
        <Link href="/" className="px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold uppercase">Go to storefront</Link>
      </div>
    );
  }
        ...
        {related.length > 0 && (
          <div className="space-y-6 pt-12 border-t border-[var(--border)]">
            ...
            <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-4">
              {related.map((p) => (
                <div key={p.id} className="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden flex flex-col p-4 space-y-3 shadow-xs hover:border-blue-500/35 transition-colors">
